...
CREATE TABLE IF NOT EXISTS listings (
  ...
  platform           TEXT CHECK (platform IN ('eBay','Etsy')),
  title              TEXT NOT NULL,
  category           TEXT,
...
CREATE OR REPLACE FUNCTION fn_order_created()
...
-- 2) Order Shipped → reserved -= qty, total -= qty
CREATE OR REPLACE FUNCTION fn_order_shipped()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION fn_return_approved()
RETURNS TRIGGER LANGUAGE plpgsql AS $$
BEGIN
...
-- Listings
INSERT INTO listings (sku, platform, title, category, status, price, cost_price, quantity_available)
VALUES
...
INSERT INTO orders (order_id,platform,order_date,buyer_name,buyer_username,sku,quantity,
  ...
  tracking_number,dispatch_deadline)
VALUES
  ('EB-29481','eBay','2024-11-15','John Smith',   'jsmith99',        'SKU-001',2,34.99,69.98,7.00,5.50,'Paid',    'Shipped',  '1Z999AA101','2024-11-17'),
  ('ET-10293','Etsy','2024-11-16','Emily Rose',   'emilyrose_crafts','SKU-002',1,22.00,22.00,2.20,4.00,'Paid',    'Processing','',          '2024-11-18'),
  ('EB-29502','eBay','2024-11-14','Mike Johnson', 'mike_j',          'SKU-003',3,14.99,44.97,4.50,6.00,'Paid',    'Delivered','1Z999AA102', '2024-11-16'),
  ('ET-10310','Etsy','2024-11-17','Sophie Turner','sophiet_art',     'SKU-004',1,55.00,55.00,5.50,8.00,'Pending', 'Pending',  '',           '2024-11-19'),
  ('EB-29520','eBay','2024-11-13','David Lee',    'dlee_buy',        'SKU-005',2,19.99,39.98,4.00,5.00,'Paid',    'Shipped',  '1Z999AA103','2024-11-15'),
  ('ET-10328','Etsy','2024-11-12','Anna White',   'anna_w',          'SKU-001',1,39.99,39.99,4.00,5.50,'Refunded','Cancelled','',           '2024-11-14')
ON CONFLICT (order_id) DO NOTHING;

-- Returns
INSERT INTO returns (order_id, sku, reason, status, refund_amount, received_date, notes)
VALUES
...
-- Done!
SELECT 'SellEasy database ready on Supabase! ✅' AS status;
